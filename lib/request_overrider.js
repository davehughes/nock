...
function RequestOverrider(req, options, interceptors, remove, cb) {
  ...
  emitError = function(error) {
    ...
    if (interceptor.handler) {
      ...
      var handlerResult = interceptor.handler(handlerRequest, handlerResponse);
      response.statusCode = handlerResult.statusCode || 200;
      response.headers = handlerResult.headers || {};
      responseBody = handlerResult.body || '';
    } else if (typeof interceptor.body === 'function') {
